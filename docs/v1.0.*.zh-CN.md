...
| `newBuildre($conditions = [], $columns = [])`                                  | `Illuminate\Database\Eloquent\Model` | 获取已经处理查询条件的`model`(**上面所有查询方法都基于这个方法**) |
...
### 7.3 通过`newBuildre($conditions)`查询后转换为`model`查询方法
